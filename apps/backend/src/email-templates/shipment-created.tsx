...
export const ShipmentCreatedEmail = ({
  ...
  return (
    <EmailLayout previewText={`Order #OSL-${orderId} is on the way!`}>
      <EmailHeader
        title="Your Order has Shipped"
        subtitle={`Order #OSL-${orderId} • Dispatched via ${carrier}`}
      />
      <Text style={{ fontSize: "14px", color: colors.onSurface, margin: "0 0 20px 0", lineHeight: "1.5" }}>
        Good news! Your liquidation cargo has been picked up by the logistics provider and is actively in transit to your specified warehouse destination.
      </Text>
      ...
      <Card title="Shipment Tracking Details">
        <Text style={detailTextStyle}><strong>Carrier:</strong> {carrier}</Text>
        <Text style={detailTextStyle}><strong>Tracking Number:</strong> {trackingNumber}</Text>
        {estimatedDelivery && (
          <Text style={detailTextStyle}><strong>Estimated Delivery:</strong> {estimatedDelivery}</Text>
        )}
      </Card>
