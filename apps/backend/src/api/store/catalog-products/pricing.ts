...
export function matchesSale(product: ProductSummary, sale?: CatalogParams["sale"]) {
  if (sale === "true") {
...
export function matchesPrice(product: ProductSummary, price?: string) {
  ...
  if (price.includes("-") && !price.includes(",")) {
    ...
    return Number.isFinite(min) && product.minPrice >= min && product.minPrice <= max
  }
...
export function sortSummaries(products: ProductSummary[], sortBy: CatalogParams["sortBy"]) {
